...
counts_wide <- dcast(final_stats, Feature.name + Degradation_Tag ~ Sample_name, value.var = "raw_count", fill = 0)
counts_wide[, feature_id := paste0(Feature.name, ":", Degradation_Tag)]
setnames(counts_wide, "Feature.name", "gene_id")
counts_wide[, Degradation_Tag := NULL]
...
pConfirm           <- matrix(res_feat$pvalue, ncol = 1);  rownames(pConfirm) <- res_feat$feature_id
stageRObj <- stageRTx(pScreen, pConfirm, tx2gene = as.data.frame(res_feat[, .(feature_id, gene_id)]))
stageRObj <- stageWiseAdjustment(stageRObj, method = "dtu", alpha = 0.05)

drim_padj_C0xrn4 <- as.data.table(getAdjustedPValues(stageRObj, order = TRUE, onlySignificantGenes = FALSE))
setnames(drim_padj_C0xrn4, c("geneID","txID","gene","transcript"), c("gene_id","feature_id","p_gene","p_transcript"))
drim_padj_C0xrn4[, gene_id        := as.character(gene_id)]
drim_padj_C0xrn4[, feature_id     := as.character(feature_id)]
drim_padj_C0xrn4[, Degradation_Tag := sub("^.*:", "", feature_id)]

mean_props <- final_stats[, .(mean_prop = mean(prop_tag, na.rm = TRUE)), by = .(Feature.name, Degradation_Tag, Genotype)]
mean_props <- dcast(mean_props, Feature.name + Degradation_Tag ~ Genotype, value.var = "mean_prop")
setnames(mean_props, c("Feature.name","Degradation_Tag"), c("gene_id","feature_tag"))
mean_props[, feature_id := paste0(gene_id, ":", feature_tag)]
setnames(mean_props, c(level1, level2), c("mean_prop_level1", "mean_prop_level2"))
mean_props[, delta_prop := abs(mean_prop_level1 - mean_prop_level2)]
mean_props[, Change     := fcase(mean_prop_level2 > mean_prop_level1, "increase",
                                 mean_prop_level2 < mean_prop_level1, "decrease",
                                 default = "no-change")]

drim_padj_C0xrn4 <- merge(drim_padj_C0xrn4,
                          mean_props[, .(gene_id, feature_id, mean_prop_level1, mean_prop_level2, delta_prop, Change)],
                          by = c("gene_id", "feature_id"), all.x = TRUE)
drim_padj_C0xrn4[, diff := ifelse(!is.na(p_gene) & p_gene <= 0.05 &
                                    !is.na(p_transcript) & p_transcript <= 0.05 &
                                    delta_prop >= 0.05, "significant", "non_significant")]

Stat_sig_C0xrn4 <- drim_padj_C0xrn4[diff == "significant"]
cat("Significant gene×tag combinations:", nrow(Stat_sig_C0xrn4), "\n")
print(Stat_sig_C0xrn4[, .N, by = Degradation_Tag])

for (tag in actual_tags) {
  safe_tag <- gsub("'", "", tag)
 # write.table(Stat_sig_C0xrn4[Degradation_Tag == tag],
#              paste0("flepseq2/fragment_analysis/diff_frag_", safe_tag, "_", comp_name, "_Stat.txt"),
 #             sep = "\t", row.names = FALSE, quote = FALSE)
}
#write.table(Stat_sig_C0xrn4,
#            paste0("flepseq2/fragment_analysis/drim_diff_frag_", comp_name, ".txt"),
#            sep = "\t", row.names = FALSE, quote = FALSE)
#write.table(Stat_sig_C0xrn4[, .N, by = Degradation_Tag],
#            paste0("flepseq2/fragment_analysis/drim_diff_frag_Nb_", comp_name, ".txt"),
#            sep = "\t", row.names = FALSE, quote = FALSE)

dynamic_gene_sets_C0xrn4 <- setNames(
  lapply(actual_tags, function(tag) strip_isoform(Stat_sig_C0xrn4[Degradation_Tag == tag, gene_id])),
  gsub("'", "p", gsub("_truncated", "_trunc", actual_tags))
)
gene_sets_C0xrn4   <- c(dynamic_gene_sets_C0xrn4, static_gene_sets)
upset_input_C0xrn4 <- fromList(gene_sets_C0xrn4)
rownames(upset_input_C0xrn4) <- unique(unlist(gene_sets_C0xrn4))

#png(paste0("flepseq2/fragment_analysis/drim_upset_gene_lists_", comp_name, ".png"), width = 1800, height = 1050, res = 150)
print(UpSetR::upset(UpSetR::fromList(gene_sets_C0xrn4), nsets = length(gene_sets_C0xrn4), order.by = "freq", text.scale = 1.3))
dev.off()

results_upset_C0xrn4 <- rbindlist(lapply(seq_len(nrow(unique(upset_input_C0xrn4))), function(i) {
  combo  <- unique(upset_input_C0xrn4)[i, ]
  active <- names(combo)[combo == 1]
  if (length(active) == 0) return(NULL)
  genes  <- rownames(upset_input_C0xrn4)[apply(upset_input_C0xrn4, 1, function(x) all(x == combo))]
  if (length(genes) == 0) return(NULL)
  data.table(count = length(genes), sets = paste(active, collapse = " & "), genes = paste(genes, collapse = ", "))
}))[order(-count)]
cat("\nTop intersections:\n"); print(head(results_upset_C0xrn4, 50))
#write.table(results_upset_C0xrn4,
...
drim_padj_xrn4dne1 <- as.data.table(getAdjustedPValues(stageRObj, order = TRUE, onlySignificantGenes = FALSE))
setnames(drim_padj_xrn4dne1, c("geneID","txID","gene","transcript"), c("gene_id","feature_id","p_gene","p_transcript"))
drim_padj_xrn4dne1[, gene_id         := as.character(gene_id)]
drim_padj_xrn4dne1[, feature_id      := as.character(feature_id)]
drim_padj_xrn4dne1[, Degradation_Tag := sub("^.*:", "", feature_id)]
...
drim_padj_xrn4dne1 <- merge(drim_padj_xrn4dne1,
                            mean_props[, .(gene_id, feature_id, mean_prop_level1, mean_prop_level2, delta_prop, Change)],
                            by = c("gene_id", "feature_id"), all.x = TRUE)
drim_padj_xrn4dne1[, diff := ifelse(!is.na(p_gene) & p_gene <= 0.05 &
                                      !is.na(p_transcript) & p_transcript <= 0.05 &
                                      delta_prop >= 0.05, "significant", "non_significant")]
