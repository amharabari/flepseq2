...
df_targets <- ds %>%
  ...
  dplyr::select(Barcode, Feature.name, PolyA, Add_tail, PolyA_length_signal) %>%
  collect() %>%
  setDT()
...
plot <- ggplot() +
  ...
  #            linewidth = 0.5) +
  # 
  # geom_vline(data = peaks_distri_2,
  #            aes(xintercept = peak_2),
  #            linetype = "dotted",
  #            color = "black",
  #            linewidth = 0.5) +
  ...
  #  subtitle = "dotted = weighted distribution",
...
plot
#pdf
ggsave("r_plots/distribution_polyA.pdf", plot, width = 7, height = 8)

#svg
ggsave("r_plots/distribution_polyA.svg", plot, width = 7, height = 8)



#r uridylation percentage
# the CJ table is nessessary to avoid inlation of 0 - not a proble for th other parameters, but as many AGIs are not uridylated it can be a problem here
total_reads <- C50[, .(total_reads = .N), by = .(AGI, rep, Genotype)]
...
all_combinations <- CJ(
  ...
  Genotype = unique(C50$Genotype))
Table_U <- C50[, .(n = .N), by = .(AGI, rep, tail, Genotype)]
Table_U <- merge(all_combinations,Table_U, by = c("AGI", "rep", "tail", "Genotype"), all.x = TRUE)
Table_U[,`:=`(tot    = sum(n),perc_U = 100 * n / sum(n)), by = .(AGI, rep, Genotype)]


Table_U <- Table_U[tail == "uridylated A-Tails"]
Table_U[is.na(perc_U), perc_U := 0]

Table_U[, mean_U := mean(perc_U, na.rm = TRUE), by = .(AGI, Genotype)]
mean_Table_U <- unique(Table_U[, .(AGI, mean_U, Genotype)])
mean_Table_U[, mean_U := round(mean_U, 2)]

Table_U <- merge(
  Table_U[, .(AGI, Genotype, rep, perc_U)],
  total_reads,
  by = c("AGI", "rep", "Genotype"),
  all.x = TRUE
)
fwrite(Table_U, file = "Table_U.csv")

#count U
tailtype_count <- C50[, .(libr = .N), by = .(Genotype, tail, rep)]
tailtype_count[, perc := libr*100/sum(libr), by = .(Genotype, rep)]
tailtype_count <- tailtype_count[tail == "uridylated A-Tails"]

#barplot U
plot <- ggplot()+
  geom_bar (data = tailtype_count, aes(x = Genotype, y = perc, fill= Genotype), stat = "identity", position = position_dodge(width = 0.9)) +
  
  facet_wrap(~rep)+
  
  scale_fill_manual(values = genotype_colors, name = NULL)+
  scale_y_continuous(limits = c(0,8), expand = c(0, NA))+
...
plot
#pdf
ggsave("r_plots/uridylation.pdf", plot , width = 6, height = 6, dpi = 1200)
#svg
ggsave("r_plots/uridylation.svg", plot , width = 6, height = 6, dpi = 1200)

# box U
box <- ggplot(Table_U, aes(x = rep, y = perc_U, color = Genotype, fill = Genotype)) +
...
rm(tailtype_count, plot, box)

## percent short reads by AGI
Table_short <- C50[, .(total_reads = .N, short_reads_tails = sum(polya_size < 30, na.rm = TRUE), perc_short = 100 * sum(polya_size < 30, na.rm = TRUE) / .N
), by = .(AGI, rep, Genotype)]
...
Table_short <- Table_short[, .(AGI, Genotype, rep, total_reads, short_reads_tails, perc_short)]

fwrite(Table_short, file = "Table_short.csv")

# short count
short_count <- C50[, .(total_reads = .N, short_reads_tails = sum(polya_size < 30 , na.rm = TRUE), perc_short = 100 * sum(polya_size < 30, na.rm = TRUE) / .N
), by = .(rep, Genotype)]
...
plot
ggsave("r_plots/barplot_short.svg", plot , width = 6, height = 6, dpi = 1200)

# box short
box <- ggplot(Table_short, aes(x = rep, y = perc_short, color = Genotype, fill = Genotype)) +
  ...
  theme(
    ...
    axis.title.x = element_blank(),
  legend.position="right", 
  strip_backround = element_rect(fill = "white", color = "grey80"), 
  strip_text = element_text(face = "bold")
  ) +
  labs(
    title = "Percentage of short tails",
    #x="",
    y= "% short")
box
ggsave("r_plots/boxshort.svg", box, width = 7, height = 6, dpi = 1200)


## percent medium reads 
Table_medium <- C50[, .(total_reads = .N, medium_reads = sum(polya_size >= 30 & polya_size <= 60 , na.rm = TRUE), perc_medium = 100 * sum(polya_size >= 35 & polya_size <= 60, na.rm = TRUE) / .N
), by = .(AGI, rep, Genotype)]
...
fwrite(Table_medium, file = "Table_medium.csv")

# medium count
medium_count <- C50[, .(total_reads = .N, medium_reads = sum(polya_size >= 30 & polya_size <= 60 , na.rm = TRUE), perc_medium = 100 * sum(polya_size >= 35 & polya_size <= 60, na.rm = TRUE) / .N
), by = .(rep, Genotype)]
...
plot
ggsave("r_plots/bar_medium.svg", plot , width = 6, height = 6, dpi = 1200)

# boxplot  medium
box <- ggplot(Table_medium, aes(x = rep, y = perc_medium, color = Genotype, fill = Genotype)) +
  ...
  labs(
    title = "Percentage of medium tails",
    #x="",
    y= "% medium")
box
ggsave("r_plots/boxmedium.svg", box, width = 7, height = 6, dpi = 1200)
...
library(lme4)
library(lmerTest)   # adds p-values and F-tests to lmer
model <- lmer(perc_medium ~ Genotype + (1|AGI) + (1|rep), 
              data = Table_medium)
# F-test for the Genotype effect (Type III)
...
plot
ggsave("r_plots/bargraph_long.svg", plot , width = 6, height = 6, dpi = 1200)

# boxplot  long
box <- ggplot(Table_long, aes(x = rep, y = perc_long, color = Genotype, fill = Genotype)) +
  
  geom_boxplot(position = position_dodge(0.9), outlier.shape = NA,  width=0.5, linewidth = 1, alpha=0.4, notch=F)+
  scale_y_continuous(limits = c(27, 90))+
  scale_color_manual(values = genotype_colors, name = "Genotype")+
  scale_fill_manual(values = genotype_colors, name = "Genotype")+
  # theme_no_grid()+
  theme_classic(base_size = 18)+
  ...
  labs(
    title = "Percentage of long tails",
    #x="",
    y= "% long")
box
ggsave("r_plots/boxlong.svg", box, width = 7, height = 6, dpi = 1200)
...
## table_AGI
table_AGI <- merge(Table_short, Table_U, by = c("AGI", "Genotype", "rep"))
table_AGI <- merge(table_AGI, Table_medium, by = c("AGI", "Genotype", "rep"))
...
table_AGI <-table_AGI[, .(AGI, Genotype, rep, median, perc_short, perc_medium, perc_long, perc_U, RPM, peak_1, peak_p,  profile_1)]
#fwrite(table_AGI, file = "table_AGI.csv")
