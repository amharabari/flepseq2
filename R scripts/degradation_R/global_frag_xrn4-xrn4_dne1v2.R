# GLOBAL DIFFERENTIAL ANALYSIS OF FRAGMENTS IN xrn4-xrn4_dne1
...
#saveRDS(bamparsed_all, file = "2_Output/bamparsed_all.rds")
#bamparsed_all <- readRDS("2_Output/bamparsed_all.rds")
...
table_before <- table(global_tag_raw$Degradation_Tag)

# enforce: 3'-truncated reads should NOT have polyA
global_tag_raw[, polya_len := as.numeric(PolyA_length_signal)]

global_tag_raw <- global_tag_raw[
  !(
...
table_after  <- table(global_tag_raw$Degradation_Tag)
...
tag_counts[, raw_count_adj    := raw_count]
tag_counts[, NonTag_reads_adj := NonTag_reads]
...
p <- ggplot(box_df, aes(x = Degradation_Tag, y = cpm, fill = Genotype)) +
...
#ggsave("fragment_analysis/global_frag_analysis_Col0_xrn4.pdf", p, width = 5, height = 5)


points <- ggplot(box_df, aes(x = Degradation_Tag, y = cpm, color = Genotype, group = Genotype)) +
...
#ggsave("fragment_analysis/global_frag_analysis_xrn4_xrn4_dne1_points.pdf", points, width = 5, height = 5)
