# RA5 GLOBAL DIFFERENTIAL ANALYSIS OF FRAGMENTS IN Col0-Col0
...
target_barcodes <- SampleTable[
  Genotype %in% c("Col0", "xrn4_dne1") & Condition == "Seedling", Barcode]

#target_barcodes <- SampleTable[
#  Genotype %in% c("Col0", "Col0") & Condition == "Seedling", Barcode] #%
...
tag_counts[, variable         := factor(Genotype, levels = c("Col0", "xrn4_dne1"))] #% 
...
setnames(mean_cpm_wide,
         old = c("Col0", "xrn4_dne1"), #%
         new = c("mean_cpm_Col0", "mean_cpm_xrn4_dne1")) #%
mean_cpm_wide[, fold_change := mean_cpm_xrn4_dne1 / mean_cpm_Col0] #%
...
print(result_table[, .(RA5_tag, Estimate, Std.Error, z.value, p.value, padj, sig,
                       mean_cpm_Col0, mean_cpm_xrn4_dne1, fold_change)]) #%
...
box_df[, Genotype        := factor(Genotype,        levels = c("Col0", "xrn4_dne1"))] #
...
ggsave("r_plots/global_RA5_Col0-xrn4.svg", points, width = 5, height = 5)
