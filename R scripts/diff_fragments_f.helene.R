...
run_comparison <- function(level1, level2, SampleTable, ds,
                           actual_tags, delta_threshold = 0.05) {
...
MUG <- fread("flepseq2/Genelists/134_genes_MUG.tsv", header=TRUE, sep="\t")
LUG <- fread("flepseq2/Genelists/33_genes_LUG.tsv", header=TRUE, sep="\t")
GMUCT_DOWN <- fread("flepseq2/Genelists/GMUCT_DOWN.txt", header=F, sep="\t")
HIGH_CONFIDENCE_TARGETS <- fread("flepseq2/Genelists/HIGH_CONFIDENCE_TARGETS.txt", header=F, sep="\t")
NAGARAJAN_DNE1_TARGETS <- fread("flepseq2/Genelists/NAGARAJAN_DNE1_TARGETS.txt", header=F, sep="\t")
TRIBE_D153N <- fread("flepseq2/Genelists/TRIBE_D153N.txt", header=F, sep="\t")
TRIBE_DNE1 <- fread("flepseq2/Genelists/TRIBE_DNE1.txt", header=F, sep="\t")
...
png(paste0("flepseq2/fragment_analysis/upset_gene_lists_", comp_name, ".png"),
